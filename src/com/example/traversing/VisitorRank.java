...
public class VisitorRank extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		String result = intent.getStringExtra(Visitorassess.EXTRA_MESSAGE);//接下来string

		try {
			rank = new JSONArray(result);

			adapter = new SimpleAdapter(VisitorRank.this, getData(),//调用下面的getdata函数
	...
	private List<Hashtable<String, Object>> getData() {
		...
		for (int i = 0; i < rank.length(); i++) {
			...
			try {
				table.put("name", rank.getJSONObject(i).getString("Name"));//第一个name是listview中的key－value结构 第二个name是jsonobject中的name
			} catch (JSONException e) {
				e.printStackTrace();
			}
			list.add(table);
